...
class TarsusOrm<T = any> implements OrmMethods<T>{
 ...
 async getList(options?: Record<string, any>, pagination?: Pagination):Promise<T[]>;
 async getList(options?: unknown, pagination?: unknown): Promise<T[]> {
    ...
    if(options){
      ...
      if(options instanceof Pagination){
        ...
        const data = await this.prepare_to_query<T[]>(_sql_)
        return data
      }// 如果走有参数的
  ...
  async prepare_to_query<T>(sql: string, args: any[] = []):Promise<T>{
    const vm = TarsusOrm;
    return new Promise( async (resolve, reject) => {
      vm.connectionPool.getConnection((err, conn) => {
        if (err) {
