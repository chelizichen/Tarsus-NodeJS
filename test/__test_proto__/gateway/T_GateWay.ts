...
class T_GateWay{
    ...
    constructor(conns:Array<{Module:string;Conn:net.SocketConnectOpts;ClientProxy:Constructor<any>}>){
    ...
    InvokeRpcMethod(Module:string,Method:string,data:any){
        const T_Client = this.T_Clients.find(v=>v.Module == Module)
        return _.invoke(T_Client,Method,data)
    }
